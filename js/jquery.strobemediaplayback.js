...
(function ($, undefined) {
  ...
  var adaptiveExperienceConfiguratorMethods = {
    ...
    rules: [
      ...
      function (context, options) {
        if (
          options.favorFlashOverHtml5Video &&
          context.hasFlashPlayerCapability
        ) {
          context.setOption(options, "useHTML5", false);
        } else {
          if (
            !options.favorFlashOverHtml5Video &&
            context.hasHTML5VideoCapability
          ) {
  ...
  AdaptiveExperienceConfigurator.prototype =
    adaptiveExperienceConfiguratorMethods;
  ...
  var strobeMediaPlaybackMethods = {
    initialize: function () {
      ...
      if (this.options.useHTML5) {
        ...
        this.options.queryString = $.fn.strobemediaplayback.generateQueryString(
          this.options
        );
        var flashvars = this.options;
        flashvars.javascriptCallbackFunction =
          "$.fn.strobemediaplayback.triggerHandler";
        var params = {
    ...
    onFlashEmbedComplete: function (event) {
      if (
        !event.success &&
        $.fn.adaptiveexperienceconfigurator.hasHTML5VideoCapability
      ) {
  ...
  $.fn.strobemediaplayback.triggerHandler = function (
    id,
    eventName,
    updatedProperties
  ) {
    ...
    if (typeof proxyMediaElement != "undefined") {
      if (typeof proxiedMediaElements[id] == "undefined") {
        ...
        proxyMediaElement.videoElement.play = jQuery.proxy(
          strobeMediaPlayback.play2,
          proxyMediaElement.strobeMediaPlayback
        );
        proxyMediaElement.videoElement.pause = jQuery.proxy(
          strobeMediaPlayback.pause,
          proxyMediaElement.strobeMediaPlayback
        );
        proxyMediaElement.videoElement.load = jQuery.proxy(
          proxyMediaElement.load,
          proxyMediaElement
        );
        proxyMediaElement.videoElement.strobeMediaPlayback =
          strobeMediaPlayback;
        monitorChanges(proxyMediaElement);
      }
      proxyMediaElement.update(
        updatedProperties,
        [eventName],
        proxyMediaElement
      );
  ...
  var writableProperties =
    "src preload currentTime defaultPlaybackRate playbackRate autoplay loop controls volume muted".split(
      " "
    );
  ...
  var isPropertyChanged = function (object, cc, propertyName) {
    return (
      ...
      object[propertyName] != cc[propertyName]
    );
  };
  ...
  var videoElementMonitorMethods = {
    ...
    update: function (properties, events, monitor) {
      ...
      for (propertyName in properties) {
        if (
          jQuery.inArray("emptied", events) < 0 &&
          monitor.cc.hasOwnProperty(propertyName) &&
          monitor.videoElement.hasOwnProperty(propertyName) &&
          monitor.cc[propertyName] != monitor.videoElement[propertyName] &&
...
org.strobemediaplayback.triggerHandler = function (
  id,
  eventName,
  updatedProperties
) {
  if (eventName == "onJavaScriptBridgeCreated") {
    ...
    if (typeof org.strobemediaplayback.proxied[id] != "undefined") {
      org.strobemediaplayback.proxied[id].update(
        updatedProperties,
        [eventName],
        org.strobemediaplayback.proxied[id]
      );
    }
  }
};
